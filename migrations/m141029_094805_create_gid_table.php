...
class m141029_094805_create_gid_table extends Migration
{
    public function safeUp()
    {
        ...
        $this->createTable('{{%post_category}}', [
            ...
            'name'        => Schema::TYPE_STRING . " NOT NULL COMMENT 'Название'",
            'id_parent'   => "TINYINT(3) NOT NULL DEFAULT 0 COMMENT 'ID родительской категории'",
        ], $tableOptions);

        $this->addPrimaryKey('pk_posts_categories', '{{%post_category}}', 'id_category');

        $this->createTable('{{%posts}}', [
            'id_post'         => Schema::TYPE_PK . " COMMENT 'ID статьи'",
            'id_category'     => "TINYINT(3) NOT NULL COMMENT 'ID категории'",
        ...
        $this->addForeignKey('fk_posts_categories', '{{%posts}}', 'id_category', '{{%post_category}}', 'id_category', 'CASCADE', 'CASCADE');
        $this->addForeignKey('fk_posts_created', '{{%posts}}', 'id_created_user', '{{%users}}', 'id_user', 'CASCADE', 'CASCADE');
        $this->addForeignKey('fk_posts_updated', '{{%posts}}', 'id_updated_user', '{{%users}}', 'id_user', 'CASCADE', 'CASCADE');
        $this->createIndex('idx_post_title', '{{%posts}}', 'title');

        $this->createTable('{{%post_comments}}', [
            'id_comment' => Schema::TYPE_BIGPK . " COMMENT 'ID комментария'",
            'id_parent'  => Schema::TYPE_BIGINT . " NOT NULL DEFAULT 0 COMMENT 'ID родительского комментария'",
            'id_post'    => Schema::TYPE_INTEGER . " COMMENT 'ID статьи'",
        ...
        $this->addForeignKey('fk_post_comments', '{{%post_comments}}', 'id_post', '{{%posts}}', 'id_post', 'CASCADE', 'CASCADE');
        $this->addForeignKey('fk_comment_user', '{{%post_comments}}', 'id_user', '{{%users}}', 'id_user', 'CASCADE', 'CASCADE');
